...
def TemplateArea(VideoRoi, frame, yf):
    # 获取逐帧匹配的范围
...
def Getdy(template_area, VideoTemplate, yf):
    # 获取y方向的位移
...
def plot(time, y_displacement, freq, amplitude_y):
    # 位移时程图像
...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        if not ret:
            break
        template_area, yf = TemplateArea(frame, 100)
        result = cv2.matchTemplate(template_area, VideoTemplate, cv2.TM_CCORR_NORMED)
        min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)
